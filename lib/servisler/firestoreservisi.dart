...
import 'package:socialapp2/modeller/duyuru.dart';
 ...
 Future<List<Gonderi>> gonderileriGetir(kullaniciId) async{
  ...
  Future <void> gonderiBegen(Gonderi gonderi,String aktifKullaniciId) async{
   ...
   DocumentSnapshot doc = await  docRef.get();
    ...
    int yeniBegeniSayisi = gonderi.begeniSayisi + 1 ;
     ...
     _firestore.collection("begeniler").document(gonderi.id).collection("gonderBegenileri").document(aktifKullaniciId).setData({});
    }
  ...
  Future <void> gonderiBegeniKaldir(Gonderi gonderi,String aktifKullaniciId) async{
    ...
    if(doc.exists){
      ...
      int yeniBegeniSayisi = gonderi.begeniSayisi - 1 ;
      docRef.updateData({"begeniSayisi":yeniBegeniSayisi});
      DocumentSnapshot docBegeni =  await _firestore.collection("begeniler").document(gonderi.id).collection("gonderBegenileri").document(aktifKullaniciId).get();
      if(docBegeni.exists){
